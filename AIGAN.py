...
def find_last_checkpoint(model_file):
    ...
    if os.path.exists(base_directory):
        ...
        if len(state_files) > 0:
            epochs = [
                state_files[i].replace(file_name, '').replace('.pth.tar', '').replace('.', '') 
            ...
            if len(epochs) > 0:
                ...
                i = np.argmax(epochs)
                return os.path.join(
                    base_directory, file_name + '.%d' % epochs[i]
                )
...
class AIGAN:
    def __init__(
        ...
        self.writer = SummaryWriter(
            './checkpoints/logs/', max_queue=100
        )
        ...
        self.attacker = LinfPGDAttack(
            ...
            rand_init=True, clip_min=box_min, 
            clip_max=box_max, targeted=self.is_targeted
        )
       ...
       # giam lr 3e-4
        ...
        self.optimizer_G = torch.optim.Adam(
            self.netG.parameters(), lr=lr
        )
        self.optimizer_D = torch.optim.Adam(
            self.netDisc.parameters(), lr=lr
        )
    ...
    def train_batch(self, x, labels):
        ...
        for _ in range(1):
            ...
            perturbation = torch.clamp(
                self.netG(x), -self.c_treshold, self.c_treshold
            )
            # perturbation = self.netG(x)
            adv_images = perturbation + x
            adv_images = torch.clamp(
                adv_images, self.box_min, self.box_max
            )
        ...
        self.writer.add_scalar(
            'iter/train/loss_D_real', 
            loss_D_real.data, 
            global_step=self.iteration
        )
        # gia tri sai lech voi anh fake
        self.writer.add_scalar(
            'iter/train/loss_D_fake', 
            loss_D_fake.data, 
            global_step=self.iteration
        )
        # gia tri sai lech voi anh fake
        self.writer.add_scalar(
            'iter/train/loss_G_fake', 
            loss_G_fake.data, 
            global_step=self.iteration
        )
        # noise gaus
        self.writer.add_scalar(
            'iter/train/loss_perturb', 
            loss_perturb.data, 
            global_step=self.iteration
        )
        self.writer.add_scalar(
    ...
    def train(self, train_dataloader, epochs):
        ...
        for epoch in range(self.start_epoch, epochs+1):
            ...
            if epoch == 120 and self.dataset_name == "mnist":
                self.optimizer_G = torch.optim.Adam(
                    self.netG.parameters(), lr=0.0001
                )
                self.optimizer_D = torch.optim.Adam(
            ...
            for i, data in tqdm(enumerate(train_dataloader, start=0)):
                ...
                if i % 50 == 0:
                    ...
                    self.writer.add_images(
                        'train/adversarial_perturbation', 
                        perturbation, 
                        global_step=epoch*len(train_dataloader)+i
                    )
                    self.writer.add_images(
                        'train/adversarial_images', 
                        images+perturbation, 
                        global_step=epoch*len(train_dataloader)+i
                    )
                    self.writer.add_images(
                        'train/adversarial_images_cl', 
                        torch.clamp(
                            images+perturbation, 
                            self.box_min, 
                            self.box_max
            ...
            self.writer.add_scalar(
                'epoch/train/loss_G_fake', 
                loss_G_fake_sum/num_batch, 
                global_step=epoch
            )
            self.writer.add_scalar(
                'epoch/train/loss_perturb', 
                loss_perturb_sum/num_batch, 
                global_step=epoch
            )
            self.writer.add_scalar(
                'epoch/train/loss_adv', 
                loss_adv_sum/num_batch, 
                global_step=epoch
            )
            self.writer.add_scalar(
                'epoch/train/loss_G', 
                loss_G_sum/num_batch, 
                global_step=epoch
            )
            self.writer.add_scalar(
                'epoch/train/fake_acc', 
                fake_acc_sum/num_batch, 
                global_step=epoch
            )
